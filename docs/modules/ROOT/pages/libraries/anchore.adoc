...
| ``add_registry_creds()``
| Add docker registry credentials to Anchore if they do not already exist. This enables Anchore Engine to pull an image from a private registry. This step can run before or within `scan_container_image`. To run within `scan_container_image`, just include set `add_registry_creds = true` in Anchore library config.
